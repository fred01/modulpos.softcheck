...
if ((!empty($save) || !empty($restore)) && $request->isPost() && check_bitrix_sessid()) {
  if (!empty($restore)) {
    ...
    if ($request->getPost('login')) {
      ...
      if ($validated) {
        Option::set(ADMIN_MODULE_NAME, "login", $login);
        Option::set(ADMIN_MODULE_NAME, "password", $password);
        Option::set(ADMIN_MODULE_NAME, "retailpoint_id", $retailpoint_id);
    ...
    <tr>
        ...
        <td width="60%">
            <input type="text"
                   ...
                   maxlength="50"
                   name="retailpoint_id"
                   value="<?=HtmlFilter::encode(Option::get(ADMIN_MODULE_NAME, 'retailpoint_id', ''));?>"
                   />
        </td>
    </tr>
